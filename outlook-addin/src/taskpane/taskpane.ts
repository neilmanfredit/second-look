...
function setupNotifyButton(item: Office.MessageRead, token: string) {
  const senderAddress = item.from?.emailAddress?.address ?? "";
  const recipientAddress = Office.context.mailbox.userProfile.emailAddress;

  // Only show for internal senders — same domain as the current user
  if (!senderAddress || !isInternalSender(senderAddress, recipientAddress)) return;
  ...
  const statusEl = document.getElementById("notify-status") as HTMLParagraphElement;

  btn.addEventListener("click", async () => {
    btn.disabled = true;
    statusEl.className = "hidden";

    const senderName = item.from?.emailAddress?.displayName ?? senderAddress;
    const subject = "A message from you may need a second look";
    const body = [
      `Hi ${senderName.split(" ")[0]},`,
      "",
      "I wanted to flag that a message you sent recently appeared to have a short review time relative to its length. This isn't a formal complaint — just a nudge to double-check the content if you haven't already.",
      "",
      "This is generated by the Second Look add-in, which tracks review time only. It cannot detect who wrote the content.",
      "",
      "Happy to discuss if useful.",
    ].join("\n");

    // Open Outlook's compose window pre-filled — user decides whether to send
    Office.context.mailbox.displayNewMessageForm({
      toRecipients: [{ displayName: senderName, emailAddress: senderAddress }],
      subject,
      htmlBody: body.replace(/\n/g, "<br/>"),
    });

    // Log the action server-side for audit trail (fire-and-forget)
    try {
      const res = await fetch(`${BACKEND_URL}/api/notifications`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ messageId: item.itemId, senderUpn: senderAddress }),
      });

      if (res.status === 429) {
        statusEl.textContent = "You've sent several notifications to this sender today.";
        statusEl.className = "error";
        btn.disabled = false;
        return;
      }
    } catch (_) {
      // Non-critical — compose window already opened
    }

    statusEl.textContent = "Compose window opened. Review and send when ready.";
    statusEl.className = "success";
    btn.disabled = false;
  });
}

function isInternalSender(senderAddress: string, recipientAddress: string): boolean {
